...
void CartesianToFrenet(const RefLine& ref, const CartesianState& cs,
    ...
    s = FindClosestS(ref, cs.x, cs.y, s_hint);
...
GeometricPath ComputeGeometricPath(const std::vector<double>& s,
    ...
    double ref_theta0 = 0.0;  // i==0의 "판단 보류" fallback용 (아래 참고)
    for (size_t i = 0; i < n; i++) {
        RefPoint rp = Interpolate(ref, s[i]);
        gp.x[i] = rp.x + d[i] * (-std::sin(rp.theta));
        gp.y[i] = rp.y + d[i] * std::cos(rp.theta);
        if (i == 0) ref_theta0 = rp.theta;
    }

    // 이 거리보다 짧은 구간은 저속으로 점들이 거의 겹쳐서 방향을 못 믿는다 -
    // "무효 처리"가 아니라 "판단 보류"로 직전 값을 그대로 이어받는다.
    //
    // 0.02m는 너무 작았다: 정지 근처 출발 시 실측으로 확인한 값 — 인접 샘플
    // 간 실제 이동량이 2~3cm만 되어도 atan2로 계산한 yaw가 보간 잡음에
    // 흔들려 kappa=0.4~0.6/m 같은 가짜 급커브로 튀었다(실제 참조선 곡률은
    // 그 지점에서 ~0.001/m 수준). 차량 폭/저속 GPS·추정 오차를 고려해
    // 신뢰 가능한 최소 이동거리로 여유 있게 높였다.
    constexpr double kMinSegmentLength = 0.15;  // [m]

    // "직전 값 그대로 이어받기"(chain-hold)는 그 자체로 새 버그를 만든다: 저속
    // 구간에서 여러 인덱스가 연달아 짧은 구간이라 직전 값을 계속 물려받다가,
    // 어느 순간 실제로 거리가 벌어지면 그 시점의 새 yaw는 "몇 스텝 전에 얼어붙은
    // 오래된 값"과의 차이로 kappa가 계산된다. 실측 재현: i=3에서 직전 값 유지로
    // yaw가 1.057에 멈춰있다가 i=4에서 갑자기 새로 계산된 값(0.314)과 섞여
    // kappa=-4.63(허용치 0.17의 27배)이 튀어나왔고, 저속에서 생성되는 모든 후보가
    // 거의 같은 지점에서 이 현상을 겪어 후보 전부가 곡률 필터에 걸리는 문제로
    // 이어졌다(실측: combined 383개 -> 곡률통과 0개). "직전 값 유지" 대신, 매
    // 인덱스마다 충분한 거리가 나올 때까지 앞뒤로 탐색범위를 넓혀 그 자리에서
    // 새로 계산하면(오래된 값을 재사용하지 않으므로) 이 문제가 생기지 않는다.
    // reach_used[i]: yaw[i]를 계산할 때 실제로 몇 스텝을 넓혔는지(정상 간격이면
    // 항상 1). kappa 계산에서 "이 인덱스의 yaw가 바로 옆 점 기준인지, 아니면
    // 저속으로 멀리까지 넓혀서 계산한 건지"를 구분하는 데 쓴다 (아래 kappa
    // 루프 설명 참고).
    std::vector<size_t> reach_used(n, 1);
    for (size_t i = 0; i < n; i++) {
        double dx = 0.0, dy = 0.0;
        bool found = false;
        size_t a = i, b = i, used_reach = 1;
        for (size_t reach = 1; reach < n; reach++) {
            a = (i >= reach) ? i - reach : 0;
            b = (i + reach < n) ? i + reach : n - 1;
            if (a == b) break;
            used_reach = reach;
            dx = gp.x[b] - gp.x[a];
            dy = gp.y[b] - gp.y[a];
            if (std::hypot(dx, dy) > kMinSegmentLength || (a == 0 && b == n - 1)) {
                found = true;
                break;
            }
        }
        // 배열 전체를 다 넓혀도 거리가 안 나오면(전체 궤적이 사실상 한 점) 방향을
        // 신뢰할 수 없으므로 참조선 heading으로 대체.
        gp.yaw[i] = found ? std::atan2(dy, dx) : ref_theta0;
        reach_used[i] = used_reach;
    }

    // kappa[i] = (yaw[i+1]-yaw[i-1]) / dist(i-1,i+1). 정상 주행속도에서는 인접
    // 점 간격이 항상 kMinSegmentLength를 넘어 yaw[i-1]/yaw[i+1]이 각자 reach=1
    // (바로 옆 점)로 계산되므로 이 비율은 항상 "일관된 4구간 기준 회전각 /
    // 2구간 거리"가 되고, 이 스케일에 맞춰 limits.max_curvature(0.1704)가
    // 튜닝되어 있다 — 정상 회전 구간의 판정 기준을 그대로 유지해야 한다.
    //
    // 문제는 저속 구간: 점들이 너무 가까워 yaw[i-1]이나 yaw[i+1]이 "훨씬 먼"
    // reach로 계산되면, 분자(그 넓은 reach 기준 회전각)와 분모(바로 옆 i-1~i+1
    // 거리)의 스케일이 서로 안 맞아 가짜 급커브가 튀었다(실측: 0.29~0.36/m,
    // 허용치 0.1704의 2배 이상 — 정지 근처에서 모든 후보가 곡률필터에 걸려
    // planner가 영구히 멈추는 문제로 이어짐). 그렇다고 kappa 계산 방식 자체를
    // 바꾸면(예: i-1,i,i+1 세 점의 국소 회전각/호길이) 정상 회전 구간에서도
    // 곡률이 실측 시 절반 가까이 낮게 나와 max_curvature와의 캘리브레이션이
    // 깨지고, 원래 걸러지던 과도한 lateral 후보가 통과해 차선 중앙 추종이
    // 흐트러지는 회귀가 실측으로 확인됐다. 그래서 계산 방식은 그대로 두고,
    // yaw[a]/yaw[b] 중 하나라도 reach>1로 계산된 경우에만(즉 저속 클러스터
    // 구간에서만) kappa를 "판단 보류"(0, 안전 쪽 통과)로 남긴다.
    for (size_t i = 0; i < n; i++) {
        const size_t a = (i == 0) ? 0 : i - 1;
        const size_t b = (i == n - 1) ? n - 1 : i + 1;
        if (reach_used[a] > 1 || reach_used[b] > 1) continue;
