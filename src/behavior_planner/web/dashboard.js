(() => {
  ...
  async function loadMap(){try{const noCache={cache:'no-store'};const [links,nodes,routeText,lightText]=await Promise.all([fetch('../src/config/link_set.json',noCache).then(r=>r.json()),fetch('../src/config/node_set.json',noCache).then(r=>r.json()),fetch('../src/config/route_links.yaml',noCache).then(r=>r.text()),fetch('../src/config/traffic_light_mock.yaml',noCache).then(r=>r.text())]);data.links=Array.isArray(links)?links:links.links;data.nodes=Array.isArray(nodes)?nodes:nodes.nodes;for(const l of data.links)data.linkById.set(l.idx,l);data.routeIds=[...routeText.matchAll(/-\s*["']?([A-Za-z0-9_]+)["']?/g)].map(x=>x[1]);data.route=data.routeIds.map(x=>data.linkById.get(x)).filter(Boolean);for(const m of lightText.matchAll(/^\s*([A-Za-z0-9_]+):\s*(RED|YELLOW|GREEN|UNKNOWN)\s*$/gm))data.lights[m[1]]={UNKNOWN:0,RED:1,YELLOW:2,GREEN:3}[m[2]];bounds={minX:Infinity,maxX:-Infinity,minY:Infinity,maxY:-Infinity};for(const l of data.links)for(const p of l.points){bounds.minX=Math.min(bounds.minX,p[0]);bounds.maxX=Math.max(bounds.maxX,p[0]);bounds.minY=Math.min(bounds.minY,p[1]);bounds.maxY=Math.max(bounds.maxY,p[1])}$('mapStatus').textContent=`MAP ${data.links.length} LINKS`;$('mapStatus').classList.add('ok');resize();fitMap()}catch(e){$('mapStatus').textContent='MAP ERROR';$('mapStatus').classList.add('bad');console.error(e)}}
